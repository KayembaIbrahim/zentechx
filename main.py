...
class TradingEngine:
    ...
    @classmethod
    def _loop(cls):
        """Auto-trading loop: scans for signals and executes for subscribed users."""
        while not cls._stop.is_set():
            try:
                ...
                if signals:
                    ...
                    # Auto-trade for all subscribed users with trades remaining
                    try:
                        with sqlite3.connect(DB_PATH) as conn:
                            conn.row_factory = sqlite3.Row
                            users = conn.execute(
                                "SELECT user_id FROM users WHERE trades_remaining > 0 AND is_restricted = 0"
                            ).fetchall()
                        ...
                        for u in users:
                            ...
                            try:
                                if is_channel_member(uid):
                                    _execute_trade_for_user(uid, uid, symbol=best["symbol"])
                                    time.sleep(1)  # Rate limit between trades
                            except Exception as e:
                                logging.error(f"Auto-trade error for {uid}: {e}")
                    except Exception as e:
                        logging.error(f"Auto-trade DB error: {e}")
            except Exception as e:
...
def handle_update(update):
    ...
    try:
        ...
        if text == "/start":
            ...
            if is_admin and u and u["trades_remaining"] <= 0:
                db_grant(uid)
                unrestrict_channel_member(uid)
                u = db_get(uid)  # refresh
            
            trades_left = u["trades_remaining"] if u else 0
            
            welcome = (
                f"👋 <b>Welcome {'Admin' if is_admin else name}!</b>\n\n"
                f"🤖 <b>Gold Eagle Trading Bot</b>\n"
                f"Trade Forex with <b>AI-powered strategies</b> 🍬⚡📈\n\n"
                f"━━━━━━━━━━━━━━━━━━\n"
            )
            
            if is_admin and trades_left > 0:
                welcome += (
                    f"✅ <b>Admin verified!</b> You have <b>{trades_left} trades</b> ready 🎫\n"
                    f"📈 Auto-trading is active for all subscribed users.\n"
                )
            else:
                welcome += (
                    f"📌 <b>How it works:</b>\n"
